...
@app.callback(
    ...
    [State("submit-button-state", "n_clicks"),State("reset_plots", "n_clicks")]+current_state+memory_state
)
def move_data_to_hidden(save_click,apply_click,clear_click, *stores):
    if clear_click>0:
        return [*[{}]*num_states,0]
...
def apply_slider_changes(n_clicks, drop_down_opts, disp_bins, freeze_buttons, adaptive_buttons, plot_buttons, exp_id, *slider_params):
    ...
    if (n_clicks>0) and (experiment_type+"_freeze" in freeze_buttons):
        ...
        non_harm_plots=[{"figure_object":{"data": [
                        ...
                        ,},"parameters":dict(zip(RV.optim_list, slider_params))} for i in range(0, len(plot_list))]
        ...
        if experiment_type!="dcv":
            for i in range(0, num_harms):
                ...
                elif experiment_type=="sinsuoidal":
