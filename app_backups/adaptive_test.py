...
changed_sv_vals=[300e-3, 3*math.pi/2,  3*math.pi/2, 30, RV_param_list["omega"], 1/200.0]
...
params=[]
#DCV.update_params(params)
#times=DCV.time_vec
#DCV.time_vec=SV.time_vec[:1]
#DCV.simulation_options["no_transient"]=0
V=DCV.e_nondim(DCV.define_voltages())
C=DCV.test_vals(params, "timeseries")
w0=[0, 0, V[0]]
#DCV.time_vec=times
wsol = odeint(DCV.current_ode_sys, w0, DCV.time_vec, atol=1e-6, rtol=1e-6)
DCV_timeseries=DCV.i_nondim(wsol[:,0])
DCV_plot_voltages=DCV.e_nondim(wsol[:,2])
plt.plot(V)
plt.plot(DCV_plot_voltages)
plt.show()
plt.plot(V, DCV_timeseries)
plt.plot(DCV_plot_voltages, DCV.i_nondim(timeseries))
plt.show()
